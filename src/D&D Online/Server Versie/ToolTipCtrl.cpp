...
void CToolTipCtrl::AddToolForWndEx(HWND hWnd, UINT uFlags, LPSTR lpszToolTip)
{
	TOOLINFO ti;
	ti.cbSize = sizeof(TOOLINFO);
	ti.uFlags = TTF_IDISHWND | uFlags;
	ti.hwnd   = hWndParent;
	ti.uId = (UINT) hWnd;
	ti.lpszText = lpszToolTip;
...
void CToolTipCtrl::AddToolForWndUsingStrRes(HWND hWnd, UINT uFlags, LPSTR lpszStrRes)
{
	TOOLINFO ti;
	ti.cbSize = sizeof(TOOLINFO);
	ti.uFlags = TTF_IDISHWND | uFlags;
